...
void NewBullet::update(float time){
	...
	case 2:
	{
		Array *enemyArray = PlayerScene::sharedPlayerScene()->getArrayForEnemy();
		for (int i = 0; i <enemyArray->count(); i++)
		{
			NewEnemy *enemy = (NewEnemy*)enemyArray->objectAtIndex(i);
		...
		if (enemy->boundingBox().intersectsRect(this->boundingBox()))
		{
			...
			if (enemy->hp <= 0)
			{
				...
				*/
				...
				//删除子弹与敌怪
				PlayerScene::sharedPlayerScene()->removeChild(enemy, true);
				this->removeFromParentAndCleanup(true);
				break;
			}
			else
